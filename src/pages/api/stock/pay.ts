...
export default async function handler(
  ...
  if (req.method === 'POST') {
    try {
      ...
      // Update the stock remaining
      const updateQuery = `UPDATE stock SET remaining = remaining - ${quantity} WHERE id = ${id}`;
      await executeQuery(pool, updateQuery);

      // Insert data into StockHistory table
      const insertQuery = `INSERT INTO StockHistory (id, quantity, username, date, type, remaining, ref, note) 
      VALUES (${id}, ${quantity}, '${username}', '${date}', 2, ${remaining - quantity}, '${ref}', '${note}')`;
